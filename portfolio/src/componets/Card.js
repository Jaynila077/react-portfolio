...
export const Card = () => {
  ...
  useEffect(() => {
    ...
    tl.to(headingRef.current, { fontSize: "48px" }, )
      .to(buttonRef.current, { scale: 16.5 }, "-=0.2")
      .to(textRef.current, { color: "#fff" }, "-=0.2")
      .to(arrowRef.current, { opacity: 1 , color:"#fff"}, "-=0.2")
      .to(picRef.current, { filter: "grayscale(0)" }, "-=0.2")
      .to(socialRef.current, { opacity: 1, y: 0, stagger: 0.1 }, "-=0.4")
